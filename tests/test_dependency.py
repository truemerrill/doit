...
class TestDependencyDb(object):
    ...
    def test_tasks(self, pdep_manager):
        pdep_manager._set("taskId_ZZZ", "dep_1", "x")
        pdep_manager._set("taskId_XXX", "dep_1", "x")
        pdep_manager._set("taskId_YYY", "dep_1", "x")
        pdep_manager.close()

        d2 = Dependency(pdep_manager.db_class, pdep_manager.name)
        d2._set("taskId_WWW", "dep_1", "x")

        tasks = d2.tasks()
        assert "taskId_XXX" in tasks
        assert "taskId_YYY" in tasks
        assert "taskId_ZZZ" in tasks
        assert "taskId_WWW" in tasks
        d2.close()
...
class TestCustomChecker(object):
    def test_not_implemented(self, dependency1):
        ...
        pytest.raises(
            NotImplementedError, checker.check_modified, None, None, None
        )
